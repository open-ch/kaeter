...
func (rp *ReleasePlan) ToFooters() (string, error) {
	...
	for _, target := range rp.Releases {
		_, err := footer.WriteString(fmt.Sprintf("%s: %s\n", ReleasePlanPrefix, target.Marshal())) //nolint:staticcheck // we are buffering this string
		if err != nil {
			return "", err
		}
	}
...
// formattingStruct is used to replace template variables in commit messages.
